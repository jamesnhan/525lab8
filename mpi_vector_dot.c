...
void Check_for_error(int local_ok, char fname[], char message[], 
      MPI_Comm comm);
void Read_n(int* n_p, int** local_n_p, int** displacements, int my_rank, int comm_sz, 
      MPI_Comm comm);
void Allocate_vectors(double** local_x_pp, double** local_y_pp,
      int local_n, MPI_Comm comm);
void Read_vector(double local_a[], int* local_n, int* displacements, int n, char vec_name[], 
      int my_rank, MPI_Comm comm);
void Print_vector(double local_b[], int* local_n, int* displacements, int n, char title[], 
      int my_rank, MPI_Comm comm);
void Parallel_vector_dot(double local_x[], double local_y[], 
      double local_z[], int local_n, MPI_Comm comm);


/*-------------------------------------------------------------------*/
int main(void) {
   int n, *local_n, *displacements;
   int comm_sz, my_rank;
   ...
   Read_n(&n, &local_n, &displacements, my_rank, comm_sz, comm);
#  ifdef DEBUG
   printf("Proc %d > n = %d, local_n = %d\n", my_rank, n, local_n[my_rank]);
#  endif
   Allocate_vectors(&local_x, &local_y, local_n[my_rank], comm);
   
   Read_vector(local_x, local_n, displacements, n, "x", my_rank, comm);
   Print_vector(local_x, local_n, displacements, n, "x is", my_rank, comm);
   Read_vector(local_y, local_n, displacements, n, "y", my_rank, comm);
   Print_vector(local_y, local_n, displacements, n, "y is", my_rank, comm);
   
   Parallel_vector_dot(local_x, local_y, &local_z, local_n[my_rank], comm);
   if (my_rank == 0)
   {
...
void Read_n(
   ...
   *displacements = malloc(comm_sz * sizeof(int));
   ...
   MPI_Barrier(comm);
   MPI_Allgather(&my_disp, 1, MPI_INT, *displacements, 1, MPI_INT, comm);
   MPI_Allgather(&my_n, 1, MPI_INT, *local_n_p, 1, MPI_INT, comm);
}  /* Read_n */
...
void Read_vector(
   ...
   if (my_rank == 0) {
      ...
      for (i = 0; i < n; i++)
         scanf("%lf", &a[i]);
      MPI_Scatterv(a, local_n, displacements, MPI_DOUBLE, local_a, local_n[my_rank], MPI_DOUBLE, 0, comm);
      free(a);
   } else {
      Check_for_error(local_ok, fname, "Can't allocate temporary vector", comm);
      MPI_Scatterv(a, local_n, displacements, MPI_DOUBLE, local_a, local_n[my_rank], MPI_DOUBLE, 0, comm);
...
void Print_vector(
   ...
   if (my_rank == 0) {
      ...
      Check_for_error(local_ok, fname, "Can't allocate temporary vector", 
            comm);
      MPI_Gatherv(local_b, local_n[my_rank], MPI_DOUBLE, b, local_n, displacements, MPI_DOUBLE,
            0, comm);
      printf("%s\n", title);
      for (i = 0; i < n; i++)
         printf("%f ", b[i]);
      ...
      Check_for_error(local_ok, fname, "Can't allocate temporary vector", 
            comm);
      MPI_Gatherv(local_b, local_n[my_rank], MPI_DOUBLE, b, local_n, displacements, MPI_DOUBLE, 0,
         comm);
   }
}  /* Print_vector */
 ...
 *   * Purpose:   Dot product a vector that's been distributed among the processes
 *    * In args:   local_x:  local storage of one of the vectors being multitplied
 *     *            local_y:  local storage for the second vector being multiplied
 *      *            local_n:  the number of components in local_x, local_y,
 *       *                      and local_z
 *        * Out arg:   local_z:  local storage for the dot product of the two vectors
 *         */
void Parallel_vector_dot(
   ...
   for (local_i = 0; local_i < local_n; local_i++)
      partial_sum += local_x[local_i] * local_y[local_i];
   MPI_Reduce(&partial_sum, local_z, 1, MPI_DOUBLE, MPI_SUM, 0, comm);
}  /* Parallel_vector_sum */
