...
void Read_vector(
   ...
   MPI_Datatype rtype;
   MPI_Type_contiguous(local_n, MPI_DOUBLE, &rtype);
   ...
   if (my_rank == 0) {
      ...
      for (i = 0; i < n; i++)
         scanf("%lf", &a[i]);
      MPI_Scatter(a, local_n, MPI_DOUBLE, local_a, 1, rtype, 0, comm);
      free(a);
   } else {
      Check_for_error(local_ok, fname, "Can't allocate temporary vector", comm);
      MPI_Scatter(a, local_n, MPI_DOUBLE, local_a, 1, rtype, 0, comm);
...
void Print_vector(
   ...
   if (my_rank == 0) {
      ...
      Check_for_error(local_ok, fname, "Can't allocate temporary vector", 
            comm);
      MPI_Gather(local_b, local_n, MPI_DOUBLE, b, 1, rtype, 0, comm);
      printf("%s\n", title);
      for (i = 0; i < n; i++)
         printf("%f ", b[i]);
      printf("\n");
